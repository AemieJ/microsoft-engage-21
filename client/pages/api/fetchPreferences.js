...
export default async (req, res) => {
    ...
    try {
        let response = await fetch(`${uri}/api/lecture/${parsed.code}`, requestOptions)
        ...
        for (let idx = 0; idx < length; ++idx) {
            let from = data1[idx].from, to = data1[idx].to
            if (Math.round(Number(from) / 1000) >= lastWeek) {
                ...
                response = await fetch(`${uri}/api/booking/preferences`, requestOptions2)
